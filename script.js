/* Algraz Engenharia — interações */
(() => {
  'use strict';

  const WHATSAPP = '5511999034048';
  ...
  form.addEventListener('submit', (e) => {
    ...
    const texto = [
      `Olá, Algraz! Meu nome é ${form.nome.value.trim()}.`,
      `WhatsApp: ${form.fone.value.trim()}`,
      form.email.value.trim() ? `E-mail: ${form.email.value.trim()}` : null,
      '',
      form.msg.value.trim(),
    ].filter(Boolean).join('\n');
  ...
  document.querySelectorAll('.faq__item').forEach((item) => {
    item.querySelector('.faq__pergunta').addEventListener('click', () => {
      ...
      document.querySelectorAll('.faq__item.is-aberto').forEach((outro) => {
        if (outro !== item) outro.classList.remove('is-aberto');
      });
